...
import { dummyChatHistory , dummyPatientData } from "../assets/dummyVariables";
...
const getChatSummaries = () =>
  Promise.resolve(
    dummyChatHistory.map(({ id, title, lastMessage, timestamp }) => ({
      id,
      title,
      lastMessage,
      timestamp,
...
// Simulate request to backend for response from GenAI model
const sendMessage = (message) => {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(
        "Thank you for sharing your symptoms. Based on what you've described, I recommend scheduling an appointment with a healthcare provider for a thorough evaluation. This is a preliminary assessment and should not replace professional medical advice."
      );
    }, 1000);
  });
};
