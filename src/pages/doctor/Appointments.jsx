...
const dummyAppointments = [
  {
    ...
    appointmentDate: '2025-06-26T09:00:00Z',
    pastReports: [
      { date: '2025-05-15', diagnosis: 'Hypertension', doctor: 'Dr. Johnson' },
      { date: '2025-03-20', diagnosis: 'Annual Checkup', doctor: 'Dr. Smith' }
    ],
  ...
  },
  {
    ...
    pastReports: [
      { date: '2025-04-10', diagnosis: 'Anxiety disorder', doctor: 'Dr. Brown' }
    ],
    consultationReport: null
  },
  {
    ...
    appointmentDate: '2025-06-26T11:00:00Z',
    pastReports: [],
    consultationReport: null
  }
];
...
export default function DoctorAppointments() {
  ...
  useEffect(() => {
    fetch('/api/appointments')
      ...
      .then(data => {
        const incomplete = data.filter(a => !a.consultationReport);
  ...
  const saveReport = (id, reportData) => {
    fetch(`/api/appointments/${id}/report`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ consultationReport: reportData })
    })
      .catch(() => {}) // ignore in dummy mode
      .finally(() => {
  ...
  return (
    <div className="min-h-screen p-6" style={{ backgroundColor: colors.lightestBlue }}>
      <div className="max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold mb-4" style={{ color: colors.darkestBlue }}>
        ...
        {filtered.length === 0 ? (
          ...
          >
            <div>
              <p className="font-semibold" style={{ color: colors.darkestBlue }}>
                {a.patientName}
              </p>
              ...
              <div className="mt-3">
                <p className="text-sm font-medium mb-1" style={{ color: colors.darkBlue }}>
                  <FileText className="inline mr-1" /> Past Reports ({a.pastReports.length})
                </p>
                    ...
                    {a.pastReports.map((r,i) => (
                      <li key={i} className="pl-2 border-l border-gray-200">
                        ...
                        <span className="text-gray-500">
                          ({new Date(r.date).toLocaleDateString()} – {r.doctor})
                        </span>
                      </li>
                    ))}
                  </ul>
